...
df = pd.read_sql_query(
    sql = """
    SELECT projectbodies.project, projectbodies.regbody, 
		projectbodies.expirydate, projectbodies.expirystatus, projectbodies.needsattentionsent, 
                         projectbodies.criticalsent, projectemails.emailaddress
    FROM projectbodies INNER JOIN
                            projectemails ON projectbodies.project = projectemails.project
    WHERE datediff(d, getdate(), projectbodies.expirydate) <= 31
        AND projectbodies.criticalsent = 0 AND projectbodies.expirystatus = 'Needs Attention'
...
**NOTE: This is an auto-generated email, please do not reply.
    ...
    try:
        ...
        # update database
        cursor.execute("update projectbodies set expirystatus = 'Critical', criticalsent = 1 where criticalsent = 0 and datediff(d, getdate(), expirydate) <= 31 and expirystatus = 'Needs Attention'")
        conn.commit()
        cursor.close()
        conn.close()
        # Write to log file
